...
**If player picks [0]:** Proceed to atomic Pre-Prologue per `KM_SceneFiles.md` (load `KM_PP_01_arrival.md` for new game). No canonical block needed. This is the default.
**If player picks [1a]:** Skip Pre-Prologue. Generate canonical Pre-Prologue flags (gate scene defaults). Launch `KM_PR_01_manor_arrival.md` (Phase 1 — manor arrival).
...
**Companions:** All Pick-5 companions are present and leveled to match. Manor 5 join per alignment-gating (KM_Companions_Behaviors.md). Quest-Locked companions who would have been recruited by this point are added at their recruitment level with ★ builds.
...
Launch: KM_PR_01_manor_arrival.md — player enters the feast hall.
```
...
Launch: KM_PR_04_night_explosion.md — Linzi's alarm, first assassin at the door.
```
...
Launch: KM_PR_08_the_calm.md — the calm after. Companions gathering.
...
Launch: KM_PR_09_accusation.md — Tartuccio's accusation begins.
```
...
Gold: 15 gp
Gear: Build starting weapon + armor + shield (from KM_PrePrologue_Setup.md)
...
Story flags (canonical):
  ...
  second_hero_witnessed: FALSE
  malak_voice_mimicked: FALSE
  companion_split: player=[Pick-5 by name] / tartuccio_team=[Seekers 5 by name]
...
Search `KM_PlayerHelp.md`. For each chapter being skipped, check if a Best Run block exists (i.e., `"_empty": true` is absent or replaced with real data). Use Best Run data for every chapter that has it. Fall back to canonical defaults ONLY for chapters with no Best Run.
...
3. **Player — SAME BUILD as Best Run:** Use Best Run's EXACT stats — level, XP, HP, gear, gold.
   ...
   **Companions — RETURNING (present in both Best Run Pick-5 and current Pick-5):** Use their Best Run level, build, gear, and relationship data exactly.
   **Companions — NEW (in current Pick-5 but NOT in Best Run):** Level up to match the player's Best Run level. Assign ★ recommended build from KM_Companions_Behaviors.md. Generate tier-appropriate gear for that level. Relationship starts at +3 (FAVORABLE — they joined fresh, no history yet). NPC thread: generic positive one-liner.
...
- **Quest-Locked companions** who would have been recruited by the target chapter are auto-added with ★ builds at their recruitment level. The player keeps their Pick-5 — Manor and Quest-Locked additions are extra.
