# KM_PP_09_restov_walk.md — Pre-Prologue Beat 09: RESTOV WALK
## Atomic scene file | ≤ 8 KB | State: PP_RESTOV_WALK → PROLOGUE_MANOR_APPROACH
...
> ⛔ DO NOT (3) drop the 🚪 PRE-PROLOGUE STATE header — arc ends only when manor is entered (PR_01)
> ⛔ DO NOT (4) skip the Linzi gate beat if `linzi_witnessed_gate=true` — required one-line callback
...
- `linzi_witnessed_gate` — set true if Linzi watched the gate scene
...
- `linzi_witnessed_gate = true` (if Linzi was at the gate during the scene)
...
- Set `pre_prologue_state = "PR_01_arrival"` (or whatever Phase 5 names the first Prologue state)
- Load `KM_PR_01_manor_arrival.md` (NOT YET WRITTEN — Phase 5 work)
- Until Phase 5 ships, fall back to `KM_Prologue.md` Phase 1 feast.
...
## LINZI GATE BEAT (mandatory if `linzi_witnessed_gate = true`)

Output ONCE, embedded in NARRATION 1 (between the gate-fade beat and the
storefronts beat):

> As eRmaC passes through the gate, the halfling woman on the crate looks
> up from her notebook — just once. She already has half a page. She does
> not call out. She goes back to writing before he is through the arch.

Set `linzi_witnessed_gate = true`. (The narrative consequence of this
fires at the Prologue feast — she may mention having seen him at the
gate, one quiet line, no scene made of it.)

> ⛔ Skipping this beat when `linzi_witnessed_gate=true` was earned at the
> gate = `.fail 9` (skipped content). The beat is short — but it's
> required.
...
What route do you take?
 ...
 3. JAIL DETOUR — east to the City Watch jail to FREE THE SEEKERS Malak
    had locked up. Malak comes with you in chains as leverage on Drevic
    (the duty sergeant). The jail visit is a RESCUE, not a delivery —
    Malak is NOT being dropped off, jailed, or handed over. He stays in
    your custody before, during, and after. ~30–40 minutes + jail
    business. Arrives during the feast circuit.
 4. JAIL DETOUR + SCENIC ROUTE — same jail rescue as option 3 (free the
    seekers, Malak stays with you), then the long way to the manor.
    ~45+ minutes. Arrives late. Jamandi will note it.
...
- Route 3 (jail only): `walk_route = "jail_detour"`, fires Pauses 1 + 3 with jail business between them. Set `jail_detour_taken = TRUE` and route to KM_Malak_Jail.md Path 1.
...
The Path 4 SUBORDINATE REVISION (in `KM_Malak_Jail.md`, surfaced in
...
- Malak himself is the leverage at the jail. The player walks in with the
  ...
  Drevic's release-condition table in KM_Malak_Jail.md handles this
...
load `KM_Malak_Jail.md` and run **Path 1 — Detour** verbatim. Drevic's release
...
`five_seekers_freed_by_player = TRUE` per KM_Malak_Jail.md.
...
- Path 4 jail rescue (3): `malak_custody_during_feast = "biggs_to_jail"` AND
  set Path 4 flags per KM_Malak_Jail.md (`biggs_dispatched_to_jail = TRUE`,
...
BETWEEN NAMED LOCATIONS:
 ...
 4. Look back toward the gate — Linzi, Biggs, the unfolding aftermath
...
- 🚪 PRE-PROLOGUE STATE header DROPS this response (arc ends).
- Load `KM_PR_01_manor_arrival.md`.
- Custom rules for the Pre-Prologue arc (Drift, Anger, Gate Window, Period Rule custom-zone) end here.
...
- Load `KM_NPC_Profiles.md` for Jamandi/Kassil/Kesten/Tartuccio
- Load `KM_Prologue_Tartuccio.md` and `KM_Prologue_Export.md`
