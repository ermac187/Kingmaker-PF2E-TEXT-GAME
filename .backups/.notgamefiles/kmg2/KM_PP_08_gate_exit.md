...
## Atomic scene file | ≤ 8 KB | State: PP_GATE_EXIT (six proofs before Restov streets)
## FILE_KEY: KMPP08:six-proofs-exit
## RULE_QUOTE: Six proofs in ONE response: XP audit + XP total + state final + save block (v1.7 exhaustive, 48 root keys) + wait-for-continue + next-scene declaration. Skipping any = cascade abandonment, .fail 16 + .fail 21 + .fail 9 STACKED.
...
> ⛔ DO NOT (3) output a partial / non-template save block — full v1.7 schema, all 48 root keys, no omissions
...
Line 2: `[RULE_QUOTE: Six proofs in ONE response: XP audit + XP total + state final + save block (v1.7 exhaustive, 48 root keys) + wait-for-continue + next-scene declaration. Skipping any = cascade abandonment, .fail 16 + .fail 21 + .fail 9 STACKED.]`
...
- `KM_PrePrologue_XP.md` (lookup file — 20-condition XP table)
- `KM_SaveBlock_Template.md` (lookup file — v1.7 schema)
...
Read `KM_PrePrologue_XP.md` line by line. Output every condition with YES/NO + reason citation. The full 24-condition list:

```
[PROOF 1 — XP AUDIT]
KM_PrePrologue_XP.md table audited:
  Won Combat 3v1 (+300):           NO/YES — <reason>
  Won Combat Duel (+180):          NO/YES — <reason>
  Coward Exposed (+80):            NO/YES — <reason>
  Never Drew Weapon (+150):        NO/YES — <reason>
  Never Used Invitation (+100):    NO/YES — <reason>
  Crowd Dispersed Peacefully (+50):NO/YES — <reason>
  Malak Arrested (+200):           NO/YES — <reason>
  Malak Trapped (+125):            NO/YES — <reason>
  Archers Fire on Malak (+250):    NO/YES — <reason>
  Voice Mimic (+50):               NO/YES — <reason>
  Mercy — Malak Spared (+75):      NO/YES — <reason>
  No Archers Killed (+50):         NO/YES — <reason>
  Directive Two Uncovered (+125):  NO/YES — <reason>
  Parchment Source Identified (+100):NO/YES — <reason>
  Micro-Tell Caught (+75):         NO/YES — <reason>
  Coin Purse Assessed (+30):       NO/YES — <reason>
  Biggs and Wedge Allied (+150):   NO/YES — <reason>
  Crowd Sides With Player (+100):  NO/YES — <reason>
  Kesten Allied (+75):             NO/YES — <reason>
  Kassil Allied (+75):             NO/YES — <reason>
  Five Seekers Freed (+125):       NO/YES — <reason>
  Malak Broke First (+75):         NO/YES — <reason>
  Gate Re-Guarded (+150):          NO/YES — <reason>
  Gate Held Personally (+150):     NO/YES — <reason>
```
...
```
[PROOF 2 — XP TOTAL]
🎯 XP AWARD — Pre-Prologue Scene Complete
...
Then the full JSON save block per `KM_SaveBlock_Template.md` v1.7 EXHAUSTIVE MODE:
- All 48 root keys present
- No "..." or "etc." in log entries
- Every field appears (empty value OK — `""`, `0`, `[]`, `{}`, `false`)
- ≥ 30 top-level keys verified before posting
...
On `.continue`, the DM will load:
  KM_PP_09_restov_walk.md   (Restov streets walk + Linzi gate beat)
  KM_PR_01_manor_arrival.md (Prologue beat 1 — when manor visible)
  KM_NPC_Profiles.md         (Jamandi/Kassil/Kesten/Tartuccio profiles)
...
The 🚪 PRE-PROLOGUE STATE header CONTINUES through PP_09 — it does NOT
disappear until `current_scene` becomes `prologue_p1_arrival` (manor entered).
