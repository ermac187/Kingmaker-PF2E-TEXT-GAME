...
## KM_FailCodes.md | Referenced by: KM_DMRules.md, KM_Tips.md, KM_Tips_B.md
...
> See KM_Commands_New.md § .FAIL 2 EXPANDED RECOVERY for the most common case.
...
.fail 2  → DM dropped/summarized/sanitized PLAYER input (NOT fake dialogue — REMOVING real input). Fix: reprint missing content, same length+.
...
.fail 8  → Story spoiler in choice menu or narration
...
.fail 20 → Outcome narrated before dice rolled
...
.fail 35 → Scene moved/ended, NPC acted unprompted, or scripted trigger missed without player input (3 sub-scopes — see § .FAIL 35 SCOPE below)
...
.fail 40 → Tips footer missing at end of response, or tip not verbatim from KM_Tips.md / KM_Tips_B.md
.fail 41 → BuildSetup prompt skipped, progress ledger missing, or STOP footer missing (see KM_BuildSetup.md § MANDATORY PROGRESS LEDGER)
.fail 42 → DM narrates eRmaC making a tactical decision in cinematic combat (see KM_CinematicCombat.md)
.fail 43 → Missing combat narration line — attack block incomplete or enemy turn without physical narration (see KM_CinematicCombat.md)
.fail 44 → Zero fellowship interaction in a combat round with 2+ allies present (see KM_CinematicCombat.md)
...
**When to fire:**
- Response ends without the `💡 TIP:` footer (per KM_Tips.md format)
- Tip content does not match the SHORT line from KM_Tips.md / KM_Tips_B.md verbatim
...
See KM_Commands_New.md § .FAIL 2 EXPANDED RECOVERY for full pattern detail.
...
- **C. Mode select / Build setup prompts skipped** (KM.txt Steps 1–2, KM_BuildSetup.md)
...
Single code, three sub-scopes. Player can fire `.fail 35` for any of:
...
All three are agency/scene-transition violations. DM should not argue which sub-scope was "more correct."
