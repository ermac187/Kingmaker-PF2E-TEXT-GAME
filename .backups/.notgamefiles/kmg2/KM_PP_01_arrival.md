...
## Atomic scene file | ≤ 8 KB | State: PP_ARRIVAL
...
2. If `build_id` is empty, missing, `null`, or `"new"`:
   ...
   - Output ONLY this directive:
     ...
     → Load KM_BuildScreen.md and present Step 1 (class menu, 1–27).
     → After class pick, load the matching KM_Builds_*.md file and present builds.
     → After build pick, run KM_BuildSetup.md prompts (ancestry → heritage → background → ability boosts → feats → spells → gear).
...
2. ☐ KM_PrePrologue_Builds.md row read — 8 brackets copied
3. ☐ KM_PrePrologue_Builds_B.md armor material confirmed
...
══════════════════════════════════════════════════════
 ...
 [2] CUT OFF — Read where he's heading and get there first (Perception / Warfare Lore)
 [3] SHOUT   — Command him to stop, loud enough for the crowd to hear (Intimidation)
 [4] APPEAL  — Call on bystanders to block his path (Diplomacy / Society)
...
After player input on the 5-option pickpocket menu:
