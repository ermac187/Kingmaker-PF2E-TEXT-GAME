...
> Companion skill modifiers + levels: pull from CURRENT sources — `KM_Signatures.md`,
> `KM_Companions_Behaviors.md`, `KM_Builds_*.md` — or estimate from the companion's
> class/lane and level. (Legacy `KM_Companions_Scaled.md` / `KM_Companions_Leveling.md`
> are not in the current set.)
> `expedition_funds`, `campaign_reserve`, and `injury_status` live in the save block.

> ⛔ **RESTORED 2026-06-02 from `.backups\.old\2`, reconciled to player decisions:**
> (1) **Hybrid cost** — Hero-Point **overflow** RUNS the mission (scales by team size +
>     distance); **gold** EQUIPS it (optional gear/provisioning boosts). § I.
> (2) **No separate currencies** — the old "Overflow"/"Hero-Point overflow" mission token is
>     GONE; all mission spending uses Hero-Point overflow (the unlimited bank), preserving
>     the 0–3 combat pool. (It was briefly renamed on first restore; now unified.)
> (3) **No `expedition_funds` pool** — mission rewards (gold/materials/items) route through
>     `.loot` (Claim / Give / Treasury-for-kingdom / Sell); deployment gold is paid from
>     your gold or the Kingdom Treasury. § X.
> (4) Skill mods: pull from current build files or DM-estimate (legacy stat files absent).
> (5) Examples use the current cross-IP roster (Keqing/Yor Forger).

---

## I. DEPLOYMENT COSTS  (HYBRID — player decision 2026-06)

**Two currencies: Hero Points RUN the mission, Gold EQUIPS it.**

### A. RUN IT — Hero-Point OVERFLOW (the unlimited bank, NOT your 0–3 combat pool)
The base cost to send a detachment. Scales with **how many** you send and **how far** — your "cost escalates by how many individuals + with what":

| Team size | Overflow cost |
|---|---|
| 1–2 companions | 1 |
| 3–4 companions | 2 |
| 5+ companions | 3 |

Distance surcharge: **+1 overflow** at 7–10 hexes, **+2** at 11+ hexes. If overflow is insufficient, Kesten warns before confirming. *(Overflow comes from clever roleplay — see § HERO POINTS. Missions spend the bank, never your precious combat pool.)*

### B. EQUIP IT — Gold (optional, "with what")
Provision the detachment with gear/supplies that assist the mission. Gold from your funds / **Kingdom Treasury** (via `.loot` treasury pool):

| Provisioning | Gold | Effect |
|---|---|---|
| Field Kit | 50 gp | +1 to Phase 1 **or** Phase 2 pool (pick at departure) |
| Full Outfit | 150 gp | +2 to one pool, or +1 to both |
| Healer's Kits (per companion) | 25 gp each | downgrade one returning injury by one severity tier |
| Specialist gear (lockpicks / climbing / alchemy) | 50 gp | +2 **if it matches the mission type** |

### Optional boosts
- **Plot Armor** — spend **1 Hero Point (from your pool)** OR **3 overflow** before departure: prevents permanent death this mission + flat **+2** to all rolls.
- **Skill Boost** — spend **1 overflow** at assignment: treat one companion as Expert in the primary skill for this mission (no effect if already Expert+).
...
*Example: Scout (Perception). Keqing +16 leads. Yor Forger +10 trained assists.
...
|---|---|---|
| +10 or better | **Critical Success** | Full reward + 1 Hero-Point overflow bonus |
...
- Any: mission completes 1 day early + **+1 Hero-Point overflow**
...
| 4 | Gold lost — lose 1d10 × 5 gp from your gold / Kingdom Treasury |
...
| **Resources** | 1d4 × (PL × 10) gp of materials (lumber, ore, provisions, valuables) → `pending_loot` |
| **Items** | any gear recovered → `pending_loot` |
| **Overflow refund** | exceptional missions partly pay for themselves: **+1 Hero-Point overflow** on Critical Success |

**All mission loot routes through `.loot`** — no separate `expedition_funds` pool. Gold, materials, and items land in `pending_loot`, and the player decides each via `.loot`: **Claim** (inventory), **Give** (to a companion, with the relationship beat), **Treasury** (Kingdom Gold/BP pool — "save it for the kingdom"), or **Sell**. The deployment-gold in § I.B is paid from your gold / the same Kingdom Treasury pool.
...
╠═════════════════════════════════════════════════════════════╣
║  REWARD       [Waypoint / Resources Xgp / +X Hero-Point overflow]      ║
╚═════════════════════════════════════════════════════════════╝
```
