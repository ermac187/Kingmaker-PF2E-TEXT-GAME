...
## Atomic scene file | State: PR_08_THE_CALM
...
## LOOKUP: KM_Prologue_Systems.md (full companion approach scripts + commitment close lines)
...
- Recruitment flags from PR_06: `hutao_recruited`, `keqing_recruited`, `yor_recruited`, `aerith_recruited`, `leliana_recruited`
...
⛔ **CHRONICLER-NPC GATE — read first.** Everything below that names Linzi (the "finds you first" order, the "6 chosen companions," the approach-script lookup) is **Linzi-run ONLY.** In a `linzi_replacement` run (`leliana_chronicler_mode: TRUE` / `linzi_primary_chronicler: FALSE`) **Linzi is NOT in this run** — she does not approach, does not get the chronicler-first beat, and is NOT one of the gathered companions. The party at the Phase 5 split is the **Active 5** (the chronicler-bard Leliana is one of them, and SHE gets the chronicler-first privilege below). Forcing Linzi in = `.fail 9`.

**Order:** Linzi finds the player first (chronicler privilege) → then Active 5 in Perception initiative order (Yor Forger, Keqing, Leliana, Hu Tao, Aerith). ⛔ **REPLACEMENT RUN:** drop Linzi; the chronicler-first beat goes to **Leliana** (the chronicler-bard), then the rest of the Active 5 in Perception order.
...
- The chosen companions are ALWAYS with the player at the Phase 5 split: **6 in a Linzi run (Active 5 + Linzi), 5 in a `linzi_replacement` run (Active 5 only — Leliana is the chronicler).** The close lines prove the outcome was earned.

> **LOOKUP:** Full approach scripts for Active 5 + Linzi → `KM_Prologue_Systems.md` (Phase D rewrite pending)
> Covers: Linzi / Hu Tao / Keqing / Yor Forger / Aerith / Leliana (Tartuccio circulates separately, not a companion)
> Also in `KM_Prologue_Systems.md`: commitment close lines for the 6, the window scene (player stays quiet)
...
[RULE_QUOTE: Six proofs on Prologue exit: XP audit + XP total + state final + save block (v1.9 exhaustive, 53 root keys) + wait-for-continue + next-scene declaration. Skipping any = cascade abandonment, .fail 16 + .fail 21 + .fail 9 STACKED.]
```
