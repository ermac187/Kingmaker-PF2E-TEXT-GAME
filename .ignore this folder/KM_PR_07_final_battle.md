...
## Atomic scene file | State: PR_07_FINAL_BATTLE
...
> ⛔⛔ **HARD GATE — DO NOT LOAD PR_07 UNLESS `manor_sweep_complete = TRUE` (all five PR_06 reunions fired).**
> PR_07 is the final battle where the **Active 5 (Hu Tao, Keqing, Yor Forger, Aerith, Leliana) fight alongside the player.** They are only present because PR_06 reunited them. If `manor_sweep_complete` is not TRUE — or any of `hutao_rejoined / keqing_rejoined / yor_rejoined / aerith_rejoined / leliana_rejoined` is not TRUE — **the manor sweep was SKIPPED. STOP.** Do NOT run the final battle with only Tartuccio + Jaethal. Return to **`KM_PR_06_manor_sweep.md`** and run the five reunion rooms, the fire/smoke, and the civilian/staff rescues (`KM_PR_NightAttack_Rooms.md § PR_06`) FIRST. The night-attack chain is PR_04 → PR_05 → **PR_06** → PR_07 — **PR_06 is NOT optional and NOT skippable.** Jumping PR_05 → PR_07 = `.fail 41` (scripted beat skipped) + `.fail 9`. (Documented live failure 2026-06-22: DM jumped to the final battle with only Tartuccio + Jaethal — the Active 5 never found, the fire never fought, no civilians/staff rescued — exactly this skip.)

> 🗺️ **EXPLORABLE ROOMS:** this beat's room set (the banquet hall = clear-gate · optional gallery tactical position · the head-table anchor) is in **`KM_PR_NightAttack_Rooms.md § PR_07`** — load alongside. The party fights TOGETHER here; dispatch is unavailable (no other rooms). Enemy budget = Assassin Leader (captain) + 3 Rift Channelers (thralls, no morale) + the Frost Giant (Jamandi's).
...
>
⛔ **CHRONICLER-NPC GATE:** "plus Linzi at your shoulder" is **Linzi-run ONLY.** In a `linzi_replacement` run (`leliana_chronicler_mode: TRUE`) Linzi is NOT in this run — she is not present, not an ally, and does NOT roll initiative. The chronicler-bard (Leliana) is already counted in the Active 5. Render the opening with the Active 5 only; drop the Linzi clause and her initiative line. Forcing her in = `.fail 9`.

> *Your companions are already with you — the five Active recruited in PR_06 reunited before the hall[, plus Linzi at your shoulder — LINZI-RUN ONLY]. No one is arriving late. Everyone is in the fight from the first beat.*
...
ALLIES: Jamandi Aldori (fighting, do not let her die)
        ...
        Active 5 companions (Hu Tao, Keqing, Yor Forger, Aerith, Leliana) — all present from Round 1
        Linzi (chronicler — combat-capable, present from Round 1) ⛔ LINZI-RUN ONLY — omit entirely in a linzi_replacement run (Leliana is already the chronicler, counted in the Active 5); no initiative line for Linzi
...
🎲 INITIATIVE
  ...
  Aerith            : d20 + 2
  Leliana           : d20 + 3
  Linzi           : d20 + 4   ⛔ LINZI-RUN ONLY — omit this line entirely in a linzi_replacement run (Linzi not present)
```
...
**Mechanical effect:** Jamandi's strikes kill the Frost Giant. Giant HP → 0 at end of narration. Full Giant XP awarded to player (allied NPC credit). Player or any companion stealing her kill = `.fail 35` — it permanently misframes Jamandi as a noble who needed saving.
