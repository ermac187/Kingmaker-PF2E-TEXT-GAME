...
## KM_Rearguard.md | Referenced by: KM_Commands.md, KM_Ch1.md-KM_Ch7.md

> ⛔ **RESTORED 2026-06-02 from `.backups\.old\2`, reconciled for current:** Kesten
> Garess is already an established NPC in the current game (security officer in the
> Prologue), so his Quest-Manager role drops in cleanly. Examples updated to the
> current cross-IP roster (Yor Forger/Keqing) — old CRPG names removed. Mission resolution
> (skill pools, injuries, costs) runs through `KM_MissionResolution.md`; its reward token
> is now "Campaign Reserve," not "Overflow." The `.quests / .scout / .delegate /
> .rearguard` commands are registered in `KM_Commands.md`.
...
- **NO HARD MINIMUM — COST AND RISK ARE THE LIMITER, not a floor.** Once the player
  has **2+ companions**, they may dispatch on missions of their own, in any number —
  up to and including sending everyone and operating SOLO. There is no "minimum must
  remain" rule. The brake is built in: **bigger detachments cost more overflow**
  (`KM_MissionResolution.md` § DEPLOYMENT COSTS — 1–2 sent = 1, 3–4 = 2, 5+ = 3, plus
  distance surcharge), and a short-handed party is the player's own risk to weigh. The
  DM does NOT block a dispatch for leaving the party thin; it may NOTE the risk in one
  line if a fight is clearly imminent ("that leaves you with no one if the camp turns")
  — a heads-up, never a veto. The player's call stands.
